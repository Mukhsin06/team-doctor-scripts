...
if (-not $NoAutostart) {
  powershell -NoProfile -ExecutionPolicy Bypass -File ".\install_autostart_windows.ps1"
}

Write-Host "✅ Windows setup completed"
Write-Host "Run agent: powershell -ExecutionPolicy Bypass -File .\run_windows.ps1"
Write-Host "Task status: Get-ScheduledTask -TaskName TeamDoctorActiveAppsAgent | Format-List"
